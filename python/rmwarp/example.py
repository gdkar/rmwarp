...
#    r = rmwarp.RMFFT(win=window);
    ...
    def diffs(n):
        plt.plot(
            np.fmod(
                (princarg(spec[n].Phi-spec[n-1].Phi)/f.hop_size - unit) - np.pi/(2 * f.hop_size)
              , np.pi/(f.hop_size/2))
        + n * 4/f.hop_size)
    def dPhi_dt_diff(n):
        return princarg(spec[n].Phi-spec[n-1].Phi)/f.hop_size
    def diff_resp(n):
        return np.fmod(
            (princarg(spec[n].Phi-spec[n-1].Phi)/f.hop_size - unit) - np.pi/(2 * f.hop_size)
          , np.pi/(f.hop_size/2)
    ...
    def do_keyed(_plt,spec, key, offset):
        if _plt is None:
            _plt = plt
        vals = np.fromiter(map(key, spec),dtype=np.float32)
...
#            )
    def do_hann_avg(spec, lo, hi,func = operator.attrgetter('dPhi_dw'), exponent=2.):
        ...
        return np.sum(func(spec)[lo:hi] * weight)/np.sum(weight)
    ...
    def do_avg_retimed(spec, slcount,slsize= 0,slspacing = 16, line_spacing=1,func=None, avg = do_magnitude_avg, avg1 = None):
        ...
        for slindex in range(slcount):
            plt.plot(
                np.fromiter((_.when  + _.lgd[slindex*slstride + slsize//2] for _ in spec),dtype=np.float32) #slindex+avg1(_,slindex* slstride,slindex*slstride + slsize,operator.attrgetter('dPhi_dw')) for _ in spec),dtype=np.float32),
               ,np.fromiter(
                    (avg(_,slindex* slstride,slindex*slstride + slsize,func) for _ in spec)
    ...
    def do_one_avg(spec):
        plt.plot(np.fromiter((_.dPhi_dw.mean() for _ in spec),dtype=np.float32))
