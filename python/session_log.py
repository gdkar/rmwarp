...
np.diff(spec[2].Phi)
plt.clf()
sames = lambda n:plt.plot(-(spec[n].dPhi_dt + spec[n-1].dPhi_dt)*0.5 + unit -np.pi/(2 * f.hop_size)+(n+0.25)* 4/f.hop_size)
diffs = lambda n: plt.plot(np.fmod((princarg(spec[n].Phi-spec[n-1].Phi)/f.hop_size) - np.pi/(2 * f.hop_size), np.pi/(f.hop_size/2)) + n * 4/f.hop_size)
unit = np.arange(r.coef) * (2 * np.pi / r.size)
